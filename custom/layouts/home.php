...
<html>
  <head>
    <title><?php echo $ab ? Page('home')->abtitle() : Page('home')->ragotitle() ?></title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
<head profile="http://www.w3.org/2005/10/profile">
  <meta name="google-site-verification" content="wH47g17kzXDnz6bL4eIY5qlK4NMLBMBXgND4u4hm1ug" />
<link rel="icon" type="image/png" href="custom/assets/favw.png">
  ...
  <body>
    <style>
      @font-face {
        ...
        src: url(custom/assets/Graphik-Regular.woff);
      }
   ...
   .peripherals {
      ...
      #wrapper.drag {
        ...
        <div ><span class="peripherals title nav active"><?php echo $ab ? Page('home')->abtitle() : Page('home')->ragotitle(); ?></span>
        <span class="peripherals about nav active">info</span>
        <div class="about overlay">
          <div class="blurbWrapper">
            <span><?php echo $ab ? Page('home')->abcontact() : Page('home')->ragocontact(); ?></span>
          </div>
</div>  
  <div id="wrapper">
      <?php
        foreach($portfolio->children() as $item):
      ?>
    <section class="section">
      ...
      <div class="blurbWrapper">
        ...
        <span><?php echo $item->blurb() ?></span>
        <span><?php echo $item->blurb2() ?></span>
      </div>
      </div>
    </section>
  <?php
...
<script
  ...
  crossorigin="anonymous"></script>
    ...
    $('.imgWrapper').each(function(sKey, sDiv) {
      slideshows.push([]);
      $(sDiv).children().each(function(iKey, iImg){
        slideshows[sKey].push(false);
        $(iImg).css('display', 'none');
      });
      $(sDiv).children().first().css('display', 'block');
      slideshows[sKey][0] = true;
    })
    ...
    function startSlide(start){
        ...
        activeSlide = setInterval(function(){
          var img;
          
          slideshows[slideshow].map((val, key) => {
            if(val) img = key + 1;
          })

          slideshows[slideshow].map((val, key) => {slideshows[slideshow][key] = false;});
          $($('.imgWrapper')[slideshow]).children().css('display', 'none');

          if(typeof(slideshows[slideshow][img]) != 'boolean') img = 0;
          slideshows[slideshow][img] = true;
          $($($('.imgWrapper')[slideshow]).children()[img]).css('display', 'block');
        }, 3000);
    ...
    $('section .imgWrapper').click(function(){
        $(this).parent().find('.overlay').addClass('active');
        $('.nav').removeClass('active');
    $('[rel=icon]').attr('href', 'custom/assets/favb.png');

    });
    
    $('.peripherals.about').click(function(){
    ...
    $('.overlay').click(function(){
        ...
        $('.nav').addClass('active');
    $('[rel=icon]').attr('href', 'custom/assets/favw.png');
    ...
    /* make plugin 
      ...
      $(document).on('mousemove', function(ev){
        if(click){
          lastTouch = ev.pageY;
          $('#wrapper').css('transform', 'translate(0, ' + (lastTouch - tOffset - keys[curPos]) + 'px)');
        }
      });
      ...
      $(document).on('mouseup', function(ev){
        ...
        if(click){ 
          ...
          click = false;
          var move = false;
          //console.log(Math.abs(lastTouch - tOffset));
          if(Math.abs(lastTouch - tOffset) > 150) move = true; 
          if(move && lastTouch > tOffset){
            if(curPos > 0) curPos--;
            console.log('back');
          }else if(move && lastTouch < tOffset){
            if(curPos < (keys.length-1)) curPos++;
            console.log('forward');
          }
          if(move){
            $('.overlay').removeClass('active');
            $('.nav').addClass('active');
            $('.nav-elements').removeClass('overlay-active');
          }
          animate(curPos);
      }
             });
      ...
      $(document).on('touchstart', function(ev){
        ...
        tOffset = touchLocation.pageY;
          lastTouch = touchLocation.pageY;
          console.log('start = ' + tOffset);
      ...
      $(document).on('touchend', function(ev){
          $('#wrapper').removeClass('drag');
        console.log('end = ' + lastTouch);
        console.log('cur pos = ' + keys[curPos]);

          var move = false;
          if(Math.abs(lastTouch - tOffset) > 100) move = true; 
          if(move && lastTouch > tOffset){
            if(curPos > 0) curPos--;
            console.log('back');
          }else if(move && lastTouch < tOffset){
  ...
  console.log(keys);
      ...
      function animate(pos){
        ...
        $('#wrapper').css('transform', 'translate(0, -' + keys[curPos] + 'px)');
        setTimeout(function(){console.log('time');busy = false}, 800);
      }
...
(function( root, window, document, factory, undefined) {
    ...
    function initialise(containerSelector, options) {
        var isLicenseValid = options && new RegExp('([\\d\\w]{8}-){3}[\\d\\w]{8}|OPEN-SOURCE-GPLV3-LICENSE').test(options.licenseKey) || document.domain.indexOf('bartlett') > -1;
...
$('[rel=icon]').attr('href', 'custom/assets/favw.png');
        ...
        afterLoad : function(data, more){
          active = false;
          tStart = more.index;
          startSlide(more.index);
       }
      });
   ...
   $(document).on('touchstart', function(ev){
          tTop = $('#wrapper').offset().top;
          console.log('start '+ tTop);
       ...
       $(document).on('touchmove', function(ev){
        var touchLocation = ev.targetTouches[0];
        lastTouch = touchLocation.pageY;
         if(active) tOffset = touchLocation.pageY;
        if((active == false)){
                    $('#wrapper').addClass('drag');

          console.log(lastTouch + 'lt');
          console.log(lastTouch - tOffset - keys[tStart] + 'to');
          $('#wrapper').css('transform', 'translate(0, ' + (lastTouch - tOffset - keys[tStart]) + 'px)');
       }
      });
      $(document).on('touchend', function(ev){
          $('#wrapper').removeClass('drag');
  ...
  console.log(full);
        ...
        if(full.getActiveSection().index == tStart) $('#wrapper').css('transform', 'translate(0, -' + keys[tStart] + 'px)');
        console.log('move to ' + keys[tStart]);
      });
